...
const Organizacoes: React.FC = () => {
  ...
  >([]);
  const [organizationTypesLoading, setOrganizationTypesLoading] =
    useState(false);
  const [organizationTypesError, setOrganizationTypesError] = useState<
    string | null
  >(null);
    ...
    condominiumService
      ...
      .catch((error) =>
        setAllocationError(
          error instanceof Error
            ? error.message
            : "Erro ao carregar tipos de rateio.",
        ),
      )
  ...
  const handleCreateOrganization = async () => {
    if (
      ...
      !organizationForm.legalName ||
      !organizationForm.doc
    ) {
      setSnackbar({
        open: true,
        message: "Preencha os campos obrigatorios da organizacao.",
        severity: "error",
      });
      return;
    }
  ...
  const handleLinkUser = async () => {
    if (!sequence.organizationId) {
      setSnackbar({
        open: true,
        message: "OrganizationId nao definido.",
        severity: "error",
      });
      return;
    }
    if (!tokenUserId) {
      setSnackbar({
        open: true,
        message: "UserId nao encontrado no token.",
        severity: "error",
      });
      return;
    }
    ...
    setLoading(true);
    try {
      const response = await organizationService.addUserToOrganization(
        sequence.organizationId,
        {
          userId: tokenUserId,
      ...
      const message =
        error instanceof Error ? error.message : "Erro ao vincular usuario.";
      setSnackbar({ open: true, message, severity: "error" });
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateCondominium = async () => {
    ...
    if (
      ...
      !condoForm.doc ||
      !condoForm.address ||
      !condoForm.neighborhood
    ) {
      setSnackbar({
        open: true,
        message: "Preencha os campos obrigatorios do condominio.",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Erro ao criar condominio.";
      setSnackbar({ open: true, message, severity: "error" });
    } finally {
      setLoading(false);
    }
  ...
  const handleUploadImage = async () => {
    if (!sequence.condominiumId) {
      setSnackbar({
        open: true,
        message: "CondominiumId nao definido.",
        severity: "error",
      });
      return;
    }
    if (!imageType || !imageFile) {
      setSnackbar({
        open: true,
        message: "Selecione o tipo e o arquivo da imagem.",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      await Promise.all(
        normalized.map(async (item) => {
          try {
            const detail =
              await condominiumImageService.getCondominiumImageById(
                item.condominiumImageId,
              );
            if (detail?.contentFile && detail?.contentType) {
              previews[item.condominiumImageId] =
      ...
      const message =
        error instanceof Error ? error.message : "Erro ao carregar imagens.";
      setSnackbar({ open: true, message, severity: "error" });
    } finally {
  ...
  const handleUseExistingId = (key: keyof SequenceState, value: string) => {
    if (!value.trim()) {
      setSnackbar({
        open: true,
        message: "Informe o ID para continuar.",
        severity: "error",
      });
      return;
    }
        ...
        {isCadastroOpen ? (
                  ...
                  <Box>
                    ...
                    >
                      {organizationTypesLoading ? (
                        ...
                        <MenuItem value="PropertyManagementCompany">
                          Administradora
                        </MenuItem>
                      )}
                    </TextField>
              ...
              >
                ...
                >
                  ...
                  <Button
                    variant="outlined"
                    onClick={() =>
                      handleUseExistingId("organizationUserId", "skipped")
                    }
                  >
                    Pular por agora
                  </Button>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <Box>
                    <TextField
                      ...
                      error={!!cepError}
                      helperText={
                        cepError || "Informe o CEP para buscar o endereco"
                      }
                      InputProps={{
                  ...
                  <Box>
                    <TextField
                      ...
                      onChange={(e) =>
                        setCondoForm((prev) => ({
                          ...prev,
                          condominiumType: e.target.value,
                        }))
                      }
                    >
                      {typesLoading ? (
                  ...
                  <Box>
                    <TextField
                      ...
                      onChange={(e) =>
                        setCondoForm((prev) => ({
                          ...prev,
                          allocationValuePerc: Number(e.target.value),
                        }))
                      }
                    />
                  </Box>
                  <Box>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={condoForm.hasBlocks}
                          onChange={(e) =>
                            setCondoForm((prev) => ({
                              ...prev,
                              hasBlocks: e.target.checked,
                            }))
                          }
                        />
                      }
                  ...
                  <Box>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={condoForm.hasWaterIndividual}
                          onChange={(e) =>
                            setCondoForm((prev) => ({
                              ...prev,
                              hasWaterIndividual: e.target.checked,
                            }))
                          }
                        />
                      }
                  ...
                  <Box>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={condoForm.hasPowerByBlock}
                          onChange={(e) =>
                            setCondoForm((prev) => ({
                              ...prev,
                              hasPowerByBlock: e.target.checked,
                            }))
                          }
                        />
                      }
                  ...
                  <Box>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={condoForm.hasGasByBlock}
                          onChange={(e) =>
                            setCondoForm((prev) => ({
                              ...prev,
                              hasGasByBlock: e.target.checked,
                            }))
                          }
                        />
                      }
              ...
              >
                ...
                >
                  ...
                  <Button
                    variant="outlined"
                    onClick={loadImages}
                    disabled={imagesLoading || loading}
                  >
                    {imagesLoading ? (
                      <CircularProgress size={20} />
                    ) : (
                  ...
                  >
                    <Box>
                      <TextField
                        ...
                        onChange={(e) =>
                          setImageFilterType(e.target.value as ImageType)
                        }
                      >
          ...
          <Paper elevation={3} sx={{ p: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Inicie a criacao de uma nova organizacao para continuar.
            </Typography>
