...
/*   const handleSaveVeiculo = (veiculoData: Omit<Veiculo, 'id'>) => {
    try {
      if (selectedVeiculo) {
        ...
        notify({ message: 'VeÌculo excluÌdo com sucesso!', type: 'success' });
      } else {
      ...
      notify({ message: 'Erro ao salvar veÌculo!', type: 'error' });
    }
  }; */

  const handleDeleteVeiculo = (veiculo: Veiculo) => {
    if (window.confirm(`Deseja realmente excluir o ve√≠culo ${veiculo.modelo} - ${veiculo.placa}?`)) {
      try {
        setVeiculos(veiculos.filter(v => v.id !== veiculo.id));
        notify({ message: 'VeÌculo excluÌdo com sucesso!', type: 'success' });
        handleCloseMenu();
      } catch {
        notify({ message: 'Erro ao excluir veÌculo!', type: 'error' });
      }
    }
  };
        ...
        <Paper elevation={3} className="veiculos-card">
          <Box className="veiculos-header">
            ...
            <Typography variant="subtitle1" className="header-subtitle">
              Controle de ve√≠culos e vagas do condom√≠nio
            </Typography>
          </Box>
              ...
              <Table>
                <TableHead className="table-header">
                  <TableRow>
                    ...
                    <TableCell className="table-header-cell">Ve√≠culo</TableCell>
                    <TableCell className="table-header-cell">Cor/Ano</TableCell>
                    <TableCell className="table-header-cell">Morador</TableCell>
                    <TableCell className="table-header-cell" align="center">A√ß√µes</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {paginatedVeiculos.length > 0 ? (
                    paginatedVeiculos.map((veiculo) => {
                      ...
                      const cor = veiculo.cor || 'N√£o definida';
                      const ano = veiculo.ano || '';
                      const moradorNome = veiculo.moradorNome || 'N√£o atribu√≠do';
