...
export interface OrganizationMeResponse {
  ...
  legalName: string;
  name: string;
  orgType: number; // 1 = administradora, 2 = administrado/autonomo
...
class OrganizationService {
  ...
  async validateOrganization(payload: OrganizationRequest) {
    try {
      ...
      return {
        valid: true,
        validations: [] as Array<{ field: string; message: string }>,
      };
    } catch (error) {
  ...
  async updateOrganization(id: string, payload: OrganizationRequest) {
    try {
      return await apiClient.put<{ organizationId: string }>(
        `${this.baseUrl}/${id}`,
        payload,
      );
    } catch (error) {
  ...
  async addUserToOrganization(
    organizationId: string,
    payload: OrganizationUserRequest,
  ) {
    try {
